...
function normalizePayload(rawData: unknown): ManualTollEntryPayload {
  const wrapper = rawData as { data?: ManualTollEntryPayload };
  return (wrapper.data ?? rawData) as ManualTollEntryPayload;
}
...
function resolveTollAmount(
  ...
  if (vehicleType === "truck") {
    ...
    if (typeof small === "number") return small;
...
async function findUserByPhone(phone: string): Promise<FirebaseFirestore.QueryDocumentSnapshot> {
...
async function findVehicleByPlate(
  ...
  for (const candidate of normalizedCandidates) {
    ...
    if (!ownerUid) return snap.docs[0];
...
export const operatorManualTollEntry = functions.https.onCall(async (rawData, context) => {
  if (!context.auth) {
  ...
  if (!data.gateId || !data.plateNumber || !data.vehicleType || !data.paymentMethod) {
    throw new functions.https.HttpsError(
      "invalid-argument",
      "gateId, plateNumber, vehicleType and paymentMethod are required"
    );
  }
