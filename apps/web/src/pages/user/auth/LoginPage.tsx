...
export const LoginPage = () => {
  ...
  // Deep link listener for native Google OAuth callback
  useEffect(() => {
    if (!isNative) return;
    let mounted = true;
  ...
  // Web GSI SDK init (web browser only)
  useEffect(() => {
  ...
  const handleGoogleNative = async () => {
    try {
      await Browser.open({ url: `${API_URL}/auth/google/redirect` });
    } catch {
      toast.error('Google login খুলতে পারছে না');
    }
  };
  ...
  return (
    <main className="relative flex min-h-screen flex-col overflow-hidden bg-[#0B1437]">
      ...
      <div className="relative z-10 mx-4 flex-1 rounded-t-3xl bg-white/[0.06] backdrop-blur-xl border border-white/10 px-6 pt-6 pb-8 shadow-2xl">
        ...
        {/* Google Sign-In */}
        {isNative ? (
          // Native mobile: open system browser for OAuth redirect flow
          <button
            onClick={handleGoogleNative}
            className="flex w-full items-center justify-center gap-3 rounded-2xl border border-white/20 bg-white/10 py-3.5 text-sm font-bold text-white hover:bg-white/15 active:scale-[0.98] transition"
          >
          ...
          // Web browser: use GSI SDK button
          <>
